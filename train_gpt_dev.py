"""Readable SOTA-like training script adapted from the decoded competition submission.

This version keeps the same overall recipe (architecture, training loop, EMA, GPTQ-style
post-training mixed quantization, compression pipeline) while remaining easier to inspect
and easier to run from the environment conventions used in the user's original script.

Compatibility additions:
- Accepts either DATA_DIR or direct DATA_PATH / TOKENIZER_PATH overrides.
- Falls back to PyTorch scaled_dot_product_attention if flash_attn_interface is missing.
  This fallback is slower and less faithful to the original submission.
"""
...
class Hyperparameters:
    ...
    direct_data_path = os.environ.get('DATA_PATH')
    direct_tokenizer_path = os.environ.get('TOKENIZER_PATH')
    ...
    datasets_dir = direct_data_path if direct_data_path else os.path.join(data_dir, 'datasets', f'fineweb10B_sp{vocab_size}')
    train_files = os.path.join(datasets_dir, 'fineweb_train_*.bin')
    val_files = os.path.join(datasets_dir, 'fineweb_val_*.bin')
    tokenizer_path = direct_tokenizer_path if direct_tokenizer_path else os.path.join(data_dir, 'tokenizers', f'fineweb_{vocab_size}_bpe.model')
